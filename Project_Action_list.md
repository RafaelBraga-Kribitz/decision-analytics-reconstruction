...
~~- [ ] All functions with random behavior: seed parameter documented, deterministic with fixed seed~~ — Module A: ``generate_population`` / ``inject_flaws`` / ``clean_population`` document ``seed`` (or env ``RANDOM_SEED``) and reproducibility; ``build_segmentation_frame`` documents ``random_state``; :class:`population_segmentation.pipeline.models.propensity.PropensityModel` documents instance ``random_state``; regression [`tests/test_stochastic_determinism_module_a_surface.py`](tests/test_stochastic_determinism_module_a_surface.py) (2026-05-12). Extend the same bar to Module B/C and any remaining helpers when you widen scope.
