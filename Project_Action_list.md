...
- [ ] Create all three module directories with implementation, not stubs: `module_a/`, `module_b/`, `module_c/`
...
- [ ] Module C: directory exists with specification.md + skeleton code
  - [ ] `module_c_forecasting_scenarios/METHODOLOGY.md` with Bayesian model specification
  - [ ] `module_c_forecasting_scenarios/src/forecast_model.py` (stub with clear interface)
- [ ] All inter-module data contracts explicitly typed: use Pydantic models or dataclasses for all module outputs
- [ ] Makefile: every target uses `poetry run`, respects virtualenv, no direct `python` calls
- [ ] CI/CD: `make test` actually runs pytest and reports coverage; passes on fresh clone
- [ ] Create `ARCHITECTURE.md` with: data flow Mermaid diagram, module dependency graph, schema contract table (5 contracts × 20 fields each), step-by-step walkthrough of one record
...
- [ ] No magic numbers: all constants in `schema.py` or `config/*.yaml`
- [ ] No unused imports, no suppressed warnings without justification comment
- [ ] Pre-commit hooks configured: Ruff, Black, Pyright, pytest smoke test
- [ ] Tests: unit tests for all `_*` helpers, integration tests for major functions, parametrized tests for edge cases
