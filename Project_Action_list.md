...
~~- [ ] All public functions have docstrings (Google or NumPy format) with: description, Args, Returns, Raises, Example~~ *(Module A: full `population_segmentation` tree audited; architecture-surface regression [`tests/test_docstrings_module_a_architecture_surface.py`](tests/test_docstrings_module_a_architecture_surface.py); `segment_reachability_aggregate`, `pipeline/model_run_manifest`, `pipeline/models/segmentation` aligned to Google sections; `poetry run pytest tests/test_docstrings_module_a_architecture_surface.py -q` + `make typecheck` 2026-05-12. Other packages / repo-wide surface still open if you extend the same bar.)*
