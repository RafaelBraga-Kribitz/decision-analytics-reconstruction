...
_LEGACY_MODULE_A_DOCKERFILE = (
    _REPO / "module_a_population_segmentation" / "docker" / "Dockerfile"
)
...
def test_docker_compose_points_at_existing_dockerfiles() -> None:
    ...
    assert mod_df == "docker/Dockerfile", f"module_a build.dockerfile must be docker/Dockerfile, got {mod_df!r}"
    assert mlf_df == "docker/mlflow.Dockerfile", f"mlflow build.dockerfile must be docker/mlflow.Dockerfile, got {mlf_df!r}"
...
def test_module_a_dockerfile_not_duplicated_under_module_package() -> None:
    assert not _LEGACY_MODULE_A_DOCKERFILE.exists(), (
        "Remove module_a_population_segmentation/docker/Dockerfile; canonical image is docker/Dockerfile"
    )
