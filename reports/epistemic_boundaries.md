...
| **`allocation_output.parquet`** (Module B) | SIMULATED | PuLP/CBC MILP solver; budget envelope $44M USD [VERIFIED — TSJE 2018 advertising pautas, T11-2]; BCP 2018 FX rate priors | Linear persuasion-adjusted contacts objective; reach caps from synthetic Module A output; FX rates from BCP corridor priors | Budget allocation is structurally sound and solver-verified (OPTIMAL status). Persuasion-adjusted contacts are a proxy metric — not audited campaign accounting. Optimal allocation would shift if real audience estimates were substituted for synthetic ones. |
...
| **`daily_posterior_forecast.parquet`** (Module C) | ILLUSTRATIVE | 4 synthetic poll-wave fixtures calibrated to verified TSJE outcome margin (+3.70 pp); GaussianRandomWalk PyMC model | Only 4 survey measurement waves over 142 campaign days; posterior is prior-dominated on poll-free days; 14 NUTS divergences noted as structural (data sparsity, not mis-specification) | The posterior mean track is directionally consistent with the verified outcome. 95% HDI is structurally wide (≈ 40 pp at election eve) due to sparse polling — correctly reflecting epistemic uncertainty, not model failure. Walk-forward validation (T9-1) will quantify out-of-sample interval coverage. |
