...
| **`battleground_probability_heatmap.geojson`** (Module C) | CALIBRATED | 2018 TSJE per-department presidential results (`geo/tsje_2018_department_results.csv`, 19 departments, reconciliation-gated against verified nationals 1,206,067 / 1,110,464); GeoJSON polygon boundaries from `paraguay_departments.geojson` (18 ADM1 features) | Hierarchical partial-pooling swing model (c_battleground_v0.4): primary estimand `poll_implied` uses unanchored national posterior × swing_j; retrodiction companion adds outcome anchor; swing_j = dept_candidate_margin_j / national_candidate_margin; win_probability_a = Φ(swing_j × m_forecast / σ_dept) where σ_dept = √((swing_j × σ_national)² + 1.5 pp²); HDI on win_prob via percentile propagation over national margin grid | win_probability_a = P(Abdo/Candidate A wins dept j). All 5 GANAR-winning departments (Alto Paraná, Central, Concepción, Cordillera, Exterior) produce win_probability_a < 0.5 at the verified national margin — calibration gate passes in CI. Absolute values depend on the poll-implied fixture posterior; swing ordering is empirically derived from real TSJE returns. Choropleth is a portfolio visualization — not a verified per-department outcome forecast or operational targeting recommendation. |
