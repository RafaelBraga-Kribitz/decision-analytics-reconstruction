...
| **MILP** | Mixed-Integer Linear Program. A mathematical solver that finds the budget allocation maximizing total persuasive contacts subject to hard feasibility constraints (caps, budget envelope, currency bands). | `module_b_resource_allocation/src/module_b_resource_allocation/models/allocation_lp.py` |
