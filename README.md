...
**Module A / Module C observability (optional):** MLflow logging is **opt-in**. Set `MLFLOW_TRACKING_URI` (and optionally `MLFLOW_EXPERIMENT_NAME`) when you want runs recorded; Module C entry points such as `python -m module_c_forecasting_scenarios.pipeline.run_tracking` and Module A export (`run_export` / `population_segmentation.pipeline`) log only when that environment is present. To view runs locally: `poetry run mlflow ui` (opens http://localhost:5000).

**Data versioning (DVC, optional):** The repo is DVC-initialized (`.dvc/`, `.dvcignore` committed) so contributors can opt in to artifact versioning without re-bootstrapping. Install the extra: `poetry install --extras tracking`. The default remote (`local-cache`) points at `../../decision-analytics-dvc-cache` — a sibling directory of the repo so the cache survives `git clean` and is not committed. To track a generated artifact: `poetry run dvc add data/processed/<file>.parquet && git add data/processed/<file>.parquet.dvc .gitignore && git commit -m "track <file> with DVC"`. To push to a real remote, override with `poetry run dvc remote modify local-cache url s3://bucket/path` (or GCS/Azure/SSH equivalents). No artifacts are tracked by default; raw inputs (`data/raw/`) stay empty because the population dataset is synthesized on demand from `module_a_population_segmentation/config/`.
