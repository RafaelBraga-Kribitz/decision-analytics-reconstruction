...
from population_segmentation.models.propensity import (
    PropensityModel,
...
def _load_canonical_bundle() -> tuple[pd.DataFrame, dict[str, float], dict[str, Any], dict, str]:
    """Load the canonical Module A export bundle from data/processed/ (no model refit)."""
    ...
    if manifest_path.is_file():
        ...
        raw_cal = manifest.get("propensity_calibration") or {}
        if isinstance(raw_cal, dict):
            calibration = raw_cal

    _, anc, _ = _load_cfg()
    return feat, seg_metrics, calibration, anc, run_id


@st.cache_data(show_spinner=False)
def _fit_propensity_for_shap(run_id: str, feat: pd.DataFrame, anc: dict) -> dict[str, Any]:
    """Lazy refit for SHAP tab only — not used for displayed propensity scores."""
    _, _, stratify_by = _load_cfg()
    return PropensityModel(random_state=42, stratify_by=stratify_by).fit_predict(feat, anc)
...
def _render_segment_explorer_tab(feat: pd.DataFrame, seg: dict[str, float]) -> None:
...
def _render_propensity_tab(feat: pd.DataFrame, anc: dict, calibration: dict[str, Any]) -> None:
    st.subheader("Propensity — Training-Target Reference Diagnostic")
    st.caption(
        "**Note:** the reliability chart compares raked propensity scores against "
        "the same synthetic training labels used during model fit (department, "
        "youth, and gender deviations from calibration anchors).  This is a "
        "reconstruction-setting diagnostic — not held-out predictive calibration."
    )
    y_true = synthetic_training_reference_labels(feat, anc, random_state=42)
...
def _render_qa_tab(feat: pd.DataFrame, use_live_rebuild: bool) -> None:
...
def _render_shap_tab(
    ...
    st.caption(
        ...
        "Canonical mode refits the model on demand for this tab only."
    )
    ...
    if prop is None:
        with st.spinner("Fitting propensity model for SHAP (lazy)..."):
            prop = _fit_propensity_for_shap(run_id, feat, anc)
...
def main() -> None:
    ...
    if use_live_rebuild:
        ...
        calibration = prop.get("calibration", {})
    else:
        feat, seg, calibration, anc, run_id = _load_canonical_bundle()
    ...
    with tab1:
        _render_segment_explorer_tab(feat, seg)

    with tab2:
        _render_propensity_tab(feat, anc, calibration)

    with tab3:
        _render_qa_tab(feat, use_live_rebuild)

    with tab4:
        prop = _render_shap_tab(feat, anc, run_id, prop)
