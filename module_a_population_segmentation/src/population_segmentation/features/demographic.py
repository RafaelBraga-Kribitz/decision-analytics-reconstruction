...
def build_demographic_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["age_bin_encoded"] = out["age_bin"].map(age_order).astype(int)

    gender_map = {"M": 1.0, "F": 0.0, "unknown": 0.5}
    out["gender_encoded"] = out["gender"].map(gender_map).fillna(0.5).astype(float)
    ...
    out["chaco_flag"] = out["department"].isin(chaco)
    out["department_region"] = out["chaco_flag"].map({True: "CHACO", False: "ORIENTAL"})

    metro = {"Central", "Asuncion"}
    out["metro_flag"] = out["department"].isin(metro)
