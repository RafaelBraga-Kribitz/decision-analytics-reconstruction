...
| A6 bootstrap ARI | 0.80 | 0.70 | Retired two-subsample method (pre-IMP-A03). Superseded by canonical `compute_bootstrap_ari`. |
| A6 bootstrap ARI (2026-07-09, IMP-A03/#55) | 0.70 (two-subsample) | ≥0.40 prod / 0.50 test | Single canonical definition: `evaluation/clustering_metrics.compute_bootstrap_ari`. Observed 0.5418 (15k) / 0.4304 (50k seed 42). **Platform note (#114):** BLAS threading can shift ARI by ~0.03 at the 50k gate; pipeline entry pins `threadpool_limits(1)` and `OPENBLAS_NUM_THREADS=1` for reproducibility. Windows seed-42 runs may read ~0.401 vs Linux ~0.430 — both clear the 0.40 ratchet floor. |
